...
// (Edge runtime hard-capped à 25s sur Hobby, insuffisant pour l'agent multi-tours)
...
- watts = consommation électrique MOYENNE 12V réaliste (pas le pic).
  ...
  Micro-onde/Four : watts de fonctionnement × usage moyen.
...
// Style Node.js serverless (req, res) — pas Web API
export default async function handler(req, res) {
  ...
  // Vercel parse automatiquement le body JSON quand Content-Type: application/json
  const body = req.body || {}
...
3. Tu peux enchaîner plusieurs search_existing et propose_equipment.`
    ...
    // Dédup finale + marquage doublons
    const seen = new Set()
    const enriched = []
